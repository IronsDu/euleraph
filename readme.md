...
* 相关存储格式
    * `<vertex_id>#<relation_type_id>` -> `<label_type_id>#<vertex_id>`
        * 通过它可以满足指定点的查询（不区分关系类型），也可以满足带关系类型的查询。
...
* 任务列表:
    - [x] 根据标签类型标识创建唯一数字id，并记录到数据库中
    - [x] 根据边类型标识创建唯一数字id，并记录到数据库中
    - [x] 根据点的id标识创建唯一的数字id，并记录到数据库中
    - [x] 使用存储层的抽象接口完成数据导入的功能
4. **实现存储层的接口**
* 依赖的任务:
    1. (1)
    2. (2)
