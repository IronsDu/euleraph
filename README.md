...
* 相关存储格式
    * `<vertex_id><direction>#<relation_type_id>` -> `<label_type_id>#<vertex_id>`
        * `direction`表示方向
...
* TODO
    * 每一条edge是否也可以分配一个唯一的数字id(int64_t)，记为`edge_id`，这可以方便图算法的实现（相关map就可以以edge 数字id作为key，要比以五元组作为key简单点）
