...
struct AdjCountQueryParams
{
    ...
    std::vector<LabelTypeId>    node_label_type_id_list;     // 节点标签（可选）
    std::vector<RelationTypeId> relation_label_type_id_list; // 边标签（可选）
    bool                        need_distinct = false;       // 是否需要去重
};
...
public:
    ...
    // @return k度邻居总数
    virtual uint64_t get_k_hop_neighbor_count(const KHopQueryParams&           params,
                                              std::shared_ptr<ReaderInterface> reader,
                                              WT_CONNECTION*                   conn) = 0;

    // @brief 共同邻居算法，根据输入的vertex_id_list，获取共同邻居的总数
    // @param 见CommonNeighborQueryParams
    // @return 共同邻居总数
    virtual int get_common_neighbor_count(const CommonNeighborQueryParams& params,
                                          std::shared_ptr<ReaderInterface> reader) = 0;
    ...
    // @return 匹配的子图实例总数
    virtual int get_subgraph_matching_count(const SubgraphMatchingParams&    params,
                                            std::shared_ptr<ReaderInterface> reader) = 0;

    virtual uint64_t
    get_adj_count(const AdjCountQueryParams& params, std::shared_ptr<ReaderInterface> reader, WT_CONNECTION* conn) = 0;
};
